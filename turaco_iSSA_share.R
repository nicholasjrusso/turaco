...
##' Load in packages
## Plotting
library(ggplot2) # revisit whether necessary
...
# Make the plots
ch.plot <- ssfPlots1(ssfCoefs.canopyHeight)
...
# All on one plot
windows()
ggarrange(ch.plot + ggtitle(bquote(~bold('A)')~'Canopy Height')) + theme(legend.position="none", plot.title = element_text(size = 18)),
          ...
          pvd15_20.plot + ggtitle(bquote(~bold('E)')~'PVD (15-20 m)')) + theme(legend.position = "none", plot.title = element_text(size = 18)),
...
RSS <- ggplot(rss, aes(x = CanopyHeight)) +
  ...
  labs(x = "Canopy Height (Scaled)",
       ...
       color = "Legend") +
  scale_color_manual(name = "Turaco", values = colors) +
  theme_light(base_size = 20) +
